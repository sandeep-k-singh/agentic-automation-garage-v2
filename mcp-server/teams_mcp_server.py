...
try:
    ...
    # Fallback Server implementation for standalone operation
    class Server:
...
async def teams_build_card_payload(ticket_key: str, ticket_summary: str, user_story: str, 
                                 acceptance_criteria: str, **kwargs) -> Dict[str, Any]:
    """Build Teams Adaptive Card payload - standalone version for testing"""
    params = TicketCardParams(
    ...
    # Return the Adaptive Card JSON structure
    return {
        "type": "message", 
        "attachments": [
            {
                ...
                "content": {
                    ...
                    "body": [
                        {
                            "type": "TextBlock",
                            "text": f"🎯 **{card.ticket_summary}**",
                            "size": "large",
                            "weight": "bolder"
                        },
                        {
                            "type": "TextBlock", 
                            "text": f"📋 {card.user_story}",
                            "wrap": True,
                            "spacing": "medium"
                        },
                        {
                            "type": "TextBlock",
                            "text": f"🎫 **Ticket**: [{card.ticket_key}]({card.jira_url})",
                            "wrap": True,
                            "size": "small"
                        }
                    ]
...
async def teams_post_approval_card(ticket_key: str, ticket_summary: str, user_story: str,
                                 acceptance_criteria: str, webhook_url: str = None, **kwargs) -> Dict[str, Any]:
    """Post approval card to Teams - standalone version for testing"""
    try:
        ...
        # In test mode or when HTTPX is not available, return success simulation
        if not DEPENDENCIES_OK:
            return {
                "success": True,
                "message": f"Card posted successfully (simulated) for {card.ticket_key}",
                "card_title": card.ticket_summary,
                "webhook_url": webhook_url[:50] + "..." if len(webhook_url) > 50 else webhook_url
            }
            
        # Actual HTTP post (when dependencies are available)
        ...
        async with httpx.AsyncClient() as client:
            ...
            return {
                ...
                "message": f"Card posted for {card.ticket_key}",
        ...
        return {
            "success": False,
            "message": f"Failed to post card: {str(e)}",
